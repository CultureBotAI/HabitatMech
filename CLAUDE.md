...
- [.claude/skills/review-yaml-category/SKILL.md](.claude/skills/review-yaml-category/SKILL.md)
  — audit a coherent habitat category or cohort without editing records,
...
FOODON, BTO and PO through OAK; the network download and changing ontology
releases are isolated in the blocking `label-correspondence` workflow. Run
`just validate-products` before pushing a grounding change, and `just report-label-drift` to see the whole list without failing.
