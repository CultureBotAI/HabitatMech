...
| The GOLD source has one node id and no counted organism, study, or biosample assertions in the aggregate path table. | `data/raw/gold_ecosystem_paths.tsv:1276` reports `gold_node_count` 1 and 0 for `organism_count`, `study_count`, `biosample_count`, and `total_assertions`. | Supported exactly. |
