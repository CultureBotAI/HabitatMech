...
def test_habitat_projection_keeps_graph_ids_when_labels_are_identical():
    from render_pages import render_graphs

    graph = {
        "nodes": [{"node_id": "a", "label": "same"}, {"node_id": "b", "label": "same"}],
        "edges": [{"subject": "a", "object": "b", "predicate": "causes"}],
    }
    projected = render_graphs([graph])[0]
    root = svg(projected["source_graph"])
    assert len(root.findall('.//s:g[@class="graph-node"]', NS)) == 2
    edge = root.find('.//s:g[@class="graph-edge"]', NS)
    assert edge.attrib["data-subject"] == "a"
    assert edge.attrib["data-object"] == "b"
